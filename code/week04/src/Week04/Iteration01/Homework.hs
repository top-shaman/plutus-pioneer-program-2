...
-- log error message if payContract fails
...
payContract = do
    ...
    Contract.handleError 
      ...
      void $ submitTx tx -- log error message if submitTx fails
    payContract
...
payTrace x y = do
    ...
    callEndpoint @"pay" h1 PayParams{ ppRecipient = pubKeyHash $ walletPubKey (Wallet 2)
                                    , ppLovelace = x }
