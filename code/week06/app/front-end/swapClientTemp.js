...
const addWallet = async (wallet, key) => {
  if(wallets.find(elem => elem.walletNumber == wallet) === undefined) { // check to see if Wallet Number is used
    await newWallet(wallet, key)
      .then(async () => {
      ...
      .then(async () => {
        console.log(wallets[walletCount-1]);
      })
      .catch(e => {
        console.log(e);
        wallets.pop();
...
checkBtn.addEventListener('click', () => {
  // console.log(isUUID);
  wallets.forEach(elem => console.log(elem));
});
...
class Wallet {
  ...
  async fetchUUID(walletNumber){
    let url = `../../W${walletNumber}.cid`; // compose URL
    const textFetch = async () => {
      let response = await fetch(url);
      if(!response.ok) {
    ...
    const extractText = async (text) => { // extract uuid from validator
      console.log(`successfully created Wallet ${walletNumber} uuid: ${await text}`);
      this.uuid = text;
    }
